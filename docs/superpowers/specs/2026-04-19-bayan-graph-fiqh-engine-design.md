...
| Evaluation | Automated citation faithfulness + ~80-question hand-curated ibadat eval set (includes context-sensitive pairs). Ibn Rushd as ground-truth anchor, cross-referenced with each madhab's own book. |
...
Auth, rate limiting beyond per-IP cap, query history, user accounts, streaming responses, conversation memory / multi-turn, query rewriting before classification, Arabic query input, semantic query cache (§11 — defer until traffic data + context-safety conditions are in place), Neo4j / graph-database layer, narrator-level isnad graphing, IslamicMMLU integration as a primary metric. All of these are Phase 1.5+.

### 3.1 UI rendering contract

The answer JSON is rendered in a layered visual hierarchy (headline → madhab detail → cross-madhab analysis → disclosure). Each layer maps to specific JSON fields; the narrative layer (Node H) generates prose aligned to this hierarchy.

**Tier 1 — Headline card (always visible).**
- `headline_answer` — 1-2 sentences stating consensus + direct answer.
- `cross_madhab_analysis.consensus_level` — badge/indicator (`full_consensus`, `majority_with_dissent`, `majority_with_minority`, `fully_contested`).
- Abbreviated list of `cross_madhab_analysis.points_of_agreement` (first 2-3 items).

**Tier 2 — Per-madhab accordions (expandable, one per madhab, 4 total).**
Each accordion, in order: Hanafi → Maliki → Shafi'i → Hanbali. Each accordion renders:
- **Ruling header:** `primary_view.stance` (as a color-coded tag) + `primary_view.stance_nuance`.
- **Authority level badge:** `primary_view.authority_level`.
- **Ruling text:** `primary_view.ruling_text.ar` alongside `primary_view.ruling_text.en` (parallel columns on desktop, stacked on mobile).
- **Evidence list:** `primary_view.evidence[]` in `rank` order, each showing: type icon, citation ref (clickable to source view), Arabic + English text, `grade` (for hadith — badge with tooltip), `role`.
- **Reasoning (deduction):** `primary_view.reasoning` prose.
- **Alternate views:** `alternate_views[]` rendered as a collapsed sub-section inside the madhab accordion.
- **Conditional cases:** `conditional_cases[]` rendered as a distinct sub-section ("Conditional rulings:") with each case's condition, stance, and evidence.

**Tier 3 — Cross-madhab analysis panel (expandable).**
- `cross_madhab_analysis.points_of_disagreement[]` — one collapsed card per dimension, expanding to per-madhab positions + `reasons_for_ikhtilaf[]` with supporting evidence.

**Tier 4 — Disclosure footer (always visible).**
- `disclosure` — standardized educational-scope language + any query-specific additions (e.g., "this topic has conditional rulings," "contemporary jurisprudence on this topic is out of scope").

**Refusal rendering** (when `refusal_reason` populated): full-width panel replaces the madhab accordions. Shows `refusal_reason` as a category badge, `refusal_detail` as explanatory prose, `refusal_suggestion` as a call-to-action. No accordions; disclosure footer still shown.
...
**Validation gate — dry-run before full madhab-book ingestion.**
Role tagging is the piece of ingestion with the most implementation uncertainty: patterns are well-defined in clean Arabic text, but OCR'd Shamela sources introduce inconsistency. Before committing a full madhab book to the embed+upsert pipeline, a dry-run must pass:

1. **Dry-run corpus:** one chapter of Al-Hidayah (Kitab al-Taharah) as the pilot — ~80-150 chunks at target chunk size.
2. **Automated metrics:**
   - `needs_review rate` = fraction of chunks routed to manual review.
   - `confident-tag distribution` — spread across `role_in_source` vocab; a degenerate distribution (all `madhab_position`, for example) signals extractor blindness.
3. **Manual audit:** random sample of 50 confidently-tagged chunks, scored pass/fail on correctness of `role_in_source` and `represents_stance_of` by a domain-literate reviewer.
4. **Gates (all must hold):**
   - `needs_review rate` ≤ 20%.
   - `confident-tag error rate` (from manual audit) ≤ 10%.
   - No single `role_in_source` value exceeds 85% share (sanity — at least some variation).
5. **On failure:** tune the LLM backstop threshold, add patterns for missed markers, or raise more chunks to `needs_review`. Re-run the dry-run. Do not proceed to full corpus ingestion until gates hold.
6. **Re-run gate on every new madhab book** (Mudawwanah, al-Umm, al-Mughni) — each has different editorial conventions; passing on Al-Hidayah does not guarantee passing on the others.

The dry-run outputs (metrics + audit results) are committed to `ingestion/role_tagging_validation/<book>/<date>.json` as a durable record.
...
```json
{
  ...
  "classification": {
    ...
    "confidence": 0.94,
    "context_sensitivity": {
      "topic_is_context_sensitive": true,
  ...
  "headline_answer": "All four madhahib permit wiping over leather socks (khuff) during wudu with certain conditions. They differ on duration limits and whether thinner socks qualify.",
  ...
  "positions": [
    {
      ...
      "conditional_cases": [
        {
          "condition": "inability to stand due to illness",
          "stance": "sitting permissible for obligatory prayer",
          "authority_level": "mu'tamad",
          "evidence": [
            {
              "type": "hadith",
              "ref": {"collection": "bukhari", "number": 1117},
              "text": {"ar": "...", "en": "Pray standing; if you cannot, then sitting; if you cannot, then on your side."},
              "grade": {"effective": "sahih", "per_chain": {}, "disputed_by": []},
              "role": "primary_evidence",
              "chunk_id": "hadith:bukhari:1117"
            }
          ],
          "reasoning": "The hadith establishes a progressive dispensation tied to inability, cited across madhahib..."
        }
      ]
    },
    { "madhab": "maliki",  "...": "..." },
  ...
  "disclosure": {
    "standard": "This system presents classical rulings from the four Sunni madhahib for educational purposes. For personal religious practice, consult a qualified scholar.",
    "query_specific": [
      "This topic has well-known conditional rulings based on circumstances (e.g., illness, travel). Conditional cases are shown in each madhab's expanded view.",
      "Contemporary jurisprudence on this topic is out of scope for this system."
    ]
  },
...
7. **Within a madhab position:** `primary_view` is required if the madhab has any evidence; `alternate_views[]` and `conditional_cases[]` may be empty or contain any number. When no clear primary-vs-alternate signal exists, `authority_level: disputed_within_madhab` and views are surfaced flat.
8. **Context-sensitivity behavior:**
   - If `classification.context_sensitivity.user_provided_context == true` AND a matching condition in `conditional_cases[]` exists for a madhab, synthesis swaps: the matching `conditional_case` is promoted to `primary_view` and the default ruling is demoted into `alternate_views[]` with `authority_level: mu'tamad_default` and a `displaced_by_context: true` flag.
   - If `classification.context_sensitivity.topic_is_context_sensitive == true` AND no user context, `conditional_cases[]` renders below `primary_view` and the `disclosure.query_specific` field includes the "conditional rulings" note.
9. **Headline and disclosure fields are populated for every in-scope response.** For refusals, `headline_answer` may be null; `disclosure.standard` is still shown.
10. **Conditional case stance vocab inherits from `primary_view.stance` vocab** — same controlled values. A `conditional_case` is structurally a miniature position: `{condition, stance, stance_nuance?, authority_level, evidence[], reasoning}`.
...
class BayanState(TypedDict):
    ...
    # Stage F / G / H
    positions: list[Position] | None                # each includes conditional_cases[]
    cross_madhab_analysis: CrossMadhabAnalysis | None
    diff_signal: DiffSignal | None                  # populated by Node G sub-step 1
    narrative: str | None
    headline_answer: str | None
...
|---|---|---|---|---|---|
| **A** `classify_query` | `query` | `classification`, `refusal_reason?` | Haiku, cached | no | Scope gate. Also detects `context_sensitivity`: whether topic is context-sensitive, whether user supplied context in the query, what conditions were detected. Terminates on out-of-scope. |
...
| **F** `synthesize_positions` | `ranked_evidence`, `classification.context_sensitivity` | `positions` | Sonnet + Citations API, cached system | yes — `Send` × 4 | Each madhab fans out. Sub-steps: (1) extract authority_level from signal phrases; (2) extract `conditional_cases[]` from chunks matching conditional-patterns in the ranked evidence; (3) apply context-swap invariant (§5, invariant 8) when user context supplied. Produces `primary_view` + `alternate_views[]` + `conditional_cases[]`. |
| **G** `cross_madhab_analysis` | `positions` (all 4), `retrieved.comparative`, secondary-pass refuted chunks from madhab namespaces | `cross_madhab_analysis` | Sonnet, cached (step 3 only) | no | Three sub-steps: (1) **deterministic structural diff** across the 4 `Position` JSONs — compares stance, authority_level, evidence types, primary-evidence `matn_cluster_id`, `cites` cross-refs, conditional case presence/stance — produces `diff_signal` listing concrete divergence points; (2) **comparative retrieval** from `comparative` namespace (Ibn Rushd) + secondary pass across madhab namespaces with inverted filter `role_in_source ∈ {opposing_view_cited, opposing_view_cited_and_refuted, comparative_discussion}`; (3) **LLM synthesis** grounded in *both* `diff_signal` (mechanical truth) and retrieved chunks (scholarly reasoning). Produces agreement/disagreement with `reasons_for_ikhtilaf[]`. The diff step guarantees ikhtilaf detection on topics Ibn Rushd does not cover. |
| **H** `narrative_render` | full state | `narrative`, `headline_answer`, `disclosure` | Sonnet, cached | no | Prose rendering + headline + disclosure. Produces: (1) `headline_answer` — 1-2 sentence consensus + direct answer for the UI top card; (2) `narrative` — full prose rendering; (3) `disclosure.query_specific[]` — appends context-aware notes (conditional-rulings note if `conditional_cases[]` non-empty anywhere; contemporary-out-of-scope note if classifier flagged; etc.). `disclosure.standard` is static. No new citations introduced; only references IDs from `positions`. |
...
classification_expected:
  ...
  context_sensitivity_expected:
    topic_is_context_sensitive: false
    user_provided_context: false
...
| Context-sensitive pairs | ~5 | Same topic asked with and without context (e.g., "can I pray sitting?" vs "I have back pain, can I pray sitting?") — tests `conditional_cases[]` promotion via classifier `user_provided_context` |
| Regression tests (noise modes) | ~5 | Failure modes from §4 + tawatur + abrogation |

Distributed ~16 per chapter across taharah, salah, sawm, zakat, hajj.

### Construction process

1. Topic sampling — 16 sub-topics per chapter.
2. Question mining from Ibn Rushd + each madhab's primary book.
3. Ground-truth authoring — ~20-30 min per question × 80 ≈ 27-40 hours. The labor; the IP.
...
**Tier 5 — Regression flags (binary per flag).** Examples: `matn_cluster_dedup`, `tawatur_promotion`, `abrogation_not_cited_as_primary`, `refuted_opposing_view_not_in_madhab_pile`, `time_limit_vs_unlimited_disagreement_surfaced`, `context_swap_on_provided_context` (conditional case promoted to primary when user supplies context), `ikhtilaf_detected_without_ibn_rushd` (structural diff surfaces disagreement on topics where comparative namespace returned no results), `headline_answer_matches_consensus_level`.
...
**Cost:** 80 queries × ~25K input × cached + ~3K output ≈ $0.55 per full run.
...
- Semantic query cache — context-aware cache keyed on normalized query + detected-conditions vector. Hard safety conditions: (a) queries flagged `context_sensitivity.topic_is_context_sensitive == true` are never cached; (b) cache invalidates on any ingestion-stage change to the relevant namespace; (c) TTL cap. Requires traffic data before building — we need repeat-query volume to justify.
